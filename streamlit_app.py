...
st.markdown('<p class="sub-header">Data-Driven Customer Retention Strategy</p>', unsafe_allow_html=True)

# Business Context Narrative
st.markdown("""
**Business Context:** PlaylistPro, a music streaming service with 75,000 subscribers, faces a critical retention challenge with a 47% annual churn rate resulting in millions in lost recurring revenue. Before this project, the company had no systematic approach to retention—no predictive analytics to identify high-risk customers, manual intuition-based targeting decisions, and unclear ROI on campaigns. This optimization system combines XGBoost machine learning (94% AUC) to predict individual churn probability with Mixed-Integer Linear Programming to determine optimal customer-action assignments. The baseline model generates $3,479 net value on $150 weekly spend (2,319% ROI), with optimal budget range of $250-400/week. The framework respects operational constraints (budget, email/push capacity) while ensuring fairness across customer segments and prioritizing high-risk, high-value customers for maximum retention impact.
""")
